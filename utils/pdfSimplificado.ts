...
export async function gerarRelatorioSimplificadoPDF(dados: PDFData) {
  try {
    const {
      nome,
      densidade,
    ...
    const hoje = new Date();
    const dataFormatada = hoje.toLocaleDateString('pt-BR'); // ex: 17/04/2025
    ...
    <html>
      ...
      <body>
        ...
        ${colunasDobras[0].length > 0 || colunasDobras[1].length > 0 ? `
    ...
    // Caminho final com nome correto
